...
import { grey, indigo, blue, teal, purple, red, orange, green } from '@material-ui/core/colors';
...
const defaultCurrentDate = new Date(2023, 2, 25, 9, 0);
{
	/* !!!change */
}
...
const AppointmentContent = withStyles(styles, { name: 'AppointmentContent' })(
	({ classes, data, ...restProps }: AppointmentContentProps) => {
		let Event = 'Event';
		console.log(data);
		// if (data.Event === 2) Event = 'General ';
		// if (data.Event === 3) Event = 'Meal';
		...
		return (
			<Appointments.AppointmentContent {...restProps} data={data}>
				<div className={classes.container}>
					...
					<div className={classNames(classes.text, classes.content)}>{`Type: ${data.type}`}</div>
...
export default function Calendar(props: { scheduleCard: ScheduleEvent[] }) {
		...
		({ onClick, classes, data, ...restProps }: AppointmentProps) => (
			<Appointments.Appointment
				...
				className={classNames({
					[classes.EventTypeAppointment]: true,
	...
	const trackColor = (track: string) => {
		if (track === 'General') return green;
		if (track === 'Food') return red;
	...
	return (
		<div className='bg-cover bg-fixed overflow-y-auto bg-[url("/img/schedule/bg.svg")] bg-no-repeat min-h-screen absolute top-0'>
			<div className="text-6xl font-black p-6 text-white mt-[64px] flex justify-center md:justify-start">
				<h1 className="bg-white text-black p-2 rounded-md border-black border-2">Schedule</h1>
			</div>
